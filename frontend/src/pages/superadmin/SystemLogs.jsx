...
export default function SystemLogs() {
  ...
  const exportToCSV = () => {
    ...
    a.setAttribute('download', `bloodconnect_system_logs_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  const getLevelBadge = (level) => {
    switch (level) {
      case 'INFO': 
        return <Badge variant="default" className="bg-blue-500/10 text-blue-400 border-blue-500/20"><Info className="h-3 w-3 mr-1" /> Info</Badge>;
      case 'WARNING': 
        return <Badge variant="warning" className="bg-amber-500/10 text-amber-400 border-amber-500/20"><AlertCircle className="h-3 w-3 mr-1" /> Warn</Badge>;
      case 'ERROR': 
        return <Badge variant="danger" className="bg-rose-500/10 text-rose-400 border-rose-500/20"><ShieldAlert className="h-3 w-3 mr-1" /> Error</Badge>;
      case 'CRITICAL': 
        return <Badge variant="primary" className="bg-rose-600 text-white border-rose-700 animate-pulse"><ShieldAlert className="h-3 w-3 mr-1" /> Critical</Badge>;
      default: 
          ...
          <table className="w-full text-left text-sm text-slate-300">
            <thead className="bg-slate-900/80 text-xs uppercase text-slate-400 border-b border-slate-800">
              <tr>
                ...
                <th scope="col" className="px-6 py-4 font-medium">Event Message</th>
                <th scope="col" className="px-6 py-4 font-medium">Context</th>
              </tr>
            </thead>
              ...
              {isLoading ? (
                ...
                filteredLogs.map((log) => (
                  <tr key={log.id} className="hover:bg-slate-800/30 transition-colors">
                    ...
                    <td className="px-6 py-4 text-slate-200 max-w-md truncate" title={log.message}>
                      {log.message}
                    </td>

...
export default function VerifyEmail() {
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      toast.error(err.response?.data?.error || "Unable to resend code");
    } finally {
            ...
            <>
              <CardHeader className="text-center border-b pt-10 pb-6 transition-colors duration-300 border-slate-200 dark:border-slate-800/50">
                ...
                <p className="mt-3 text-sm leading-relaxed px-2 transition-colors duration-300 text-slate-600 dark:text-slate-400">
                  ...
                  <span className="font-semibold transition-colors duration-300 text-slate-900 dark:text-slate-200">
                    {email}
                  </span>
                </p>
              </CardHeader>
              ...
              <CardContent className="pt-8 px-8 pb-8">
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div className="space-y-2">
                    ...
                    <div className="relative group">
                      ...
                      <Input
                        ...
                        placeholder="000000"
                        className="pl-14 h-16 text-center text-3xl tracking-[0.5em] font-mono font-bold transition-all duration-300 focus:border-emerald-500 focus:ring-emerald-500/20 bg-white border-slate-200 dark:bg-slate-950/50 dark:border-slate-700"
                  ...
                  <Button
                    ...
                    className="w-full py-6 text-base font-semibold rounded-xl shadow-md hover:shadow-lg transition-all gap-2 bg-emerald-600 hover:bg-emerald-700 dark:bg-emerald-600 dark:hover:bg-emerald-500 dark:shadow-lg dark:hover:shadow-emerald-500/20"
                  >
                    {status === "loading" ? (
                      <>
                ...
                <div className="text-center mt-6">
                  {countdown > 0 ? (
                    ...
                    <button
                      ...
                      className="text-sm font-semibold transition-colors duration-300 text-emerald-600 hover:text-emerald-700 dark:text-emerald-400 dark:hover:text-emerald-300"
                    >
                      {isResending
                        ? "Sending..."
                ...
                <div className="mt-8 pt-6 border-t text-center transition-colors duration-300 border-slate-200 dark:border-slate-800/80">
                  <Link
                    to="/login"
                    className="text-sm font-semibold transition-colors duration-300 text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white"
                  >
                    ← Back to Login
                  </Link>
                </div>
